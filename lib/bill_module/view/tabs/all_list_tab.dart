...
class AllListPage extends StatefulWidget {
  ...
  @override
  State<AllListPage> createState() => _AllListState();
}

class _AllListState extends State<AllListPage> {
  @override
  void initState() {
    super.initState();
    // Load all bills without filtering
    final billBloc = BlocProvider.of<BillListBloc>(context);
    billBloc.add(BillTabClicked(index: 0));
  }
          ...
          Navigator.push(
            ...
            MaterialPageRoute(
              builder: (context) => BillDetailsPage(
                dlSaleId: bill.dlsaleid.toString(),
                dlEmpId: bill.dlempid.toString(), customerId: bill.customerID, riderName: logindata.getString("userName")!,
                // bill: {
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          // Text(
                          //    bill.dlassignedat.toString().split(' ')[0],
                          //   style: TextStyle(
                          //     fontSize: 12,
                          //     color: Colors.grey[600],
                          //   ),
                    ...
                    Column(
                      children: [
                        ...
                        //       locationBloc.add(GetLocation(customerId: bill.customerID));
          ...
          return RefreshIndicator(
            onRefresh: () async {
              BlocProvider.of<BillListBloc>(context)
                  .add(BillTabClicked(index: 0));
            },
            child: ListView.builder(
              itemCount: state.billlitmodel.length,
              itemBuilder: (context, index) {
                return _buildBillCard(
                  state.billlitmodel[index],
                  responsiveData,
                );
              },
            ),
