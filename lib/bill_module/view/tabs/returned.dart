...
class _ReturnedTabState extends State<ReturnedTab> {
  ...
  void initState() {
    super.initState();
    // Load bills with returned status
    final billBloc = BlocProvider.of<BillListBloc>(context);
    billBloc.add(BillTabClicked(index: 4)); // Index 4 for returned status
  }

  Color getRibbonColor(int status) {
    return Colors.orange; // Always orange for returned items
  }
                        ...
                        children: [
                          ...
                          Row(
                            children: [
                              ...
                              Text(
                                '${bill.quantity}/${bill.amountreceived.toStringAsFixed(2)}',
                                style: const TextStyle(
                                  fontSize: 14,
                                  fontWeight: FontWeight.bold,
                          ...
                          Text(
                            'Returned: ${bill.dlassignedat.toString().split(' ')[0]}',
                            style: TextStyle(
                              fontSize: 12,
                              color: Colors.grey[600],
                            ),
                          ),
                          if (bill.dlremarks?.isNotEmpty ?? false)
                            Padding(
    ...
    return BlocBuilder<BillListBloc, BillListState>(
      builder: (context, state) {
        if (state is BillListLoading) {
          ...
          // Filter for only returned bills
         // Change this line in _ReturnedTabState
final returnedBills = state.billlitmodel.where((bill) => bill.dlstatus == BillStatus.RETURNED).toList();

          if (returnedBills.isEmpty) {
            return const Center(
              child: Text('No returned bills available'),
            );
          }
          ...
          return RefreshIndicator(
            onRefresh: () async {
              BlocProvider.of<BillListBloc>(context)
                  .add(BillTabClicked(index: 4));
            },
            child: ListView.builder(
              itemCount: returnedBills.length,
